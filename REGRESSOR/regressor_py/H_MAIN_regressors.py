...
print(f"This is allind shape: {allind.shape}")
print()
allnames=['Mean SNRCoil','Mean FWD','Scan interval(wk)','Scan_1 age(PMA)','Mean Headsize']
...
plt.legend([f"Mean SNRCoil (p<{str(round(pvalues[1],3))})", \
    f"Mean FWD (p<{str(round(pvalues[2],3))})", f"Scan interval(wk) (p<{str(round(pvalues[3],3))})", \
        f"Scan_1 age(PMA) (p<{str(round(pvalues[4],3))})", f"Mean Headsize (p<{str(round(pvalues[5],3))})"], \
            loc='lower center', labelcolor=['g','b','c','m','r'], fontsize=24)
plt.title(f"Relationship to Connectome Stability \n Regression Coeffiecients", fontsize=30)
plt.xticks([])
plt.yticks([])
